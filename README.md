...
GalChat 是一款基于 Flutter 的 AI Galgame 风格聊天应用。你将以高中生的身份，与伪装成转学生的安卓猫娘 AI **初雪** 在现代校园日常中展开互动。AI 的每个回复都包含**场景描写**、**角色对话**和 **A/B/C/D 分支选项**，营造出经典的 Galgame 体验。

> 🎮 聊天即游戏 —— 你的每一次对话选择，都会影响好感度、剧情分支和角色关系。

## 功能特性

- 💬 **AI 角色扮演** — 与初雪自由聊天，AI 返回场景叙述 + 对话 + 分支选项
- 🎯 **Galgame 选项系统** — 每回合 2-4 个可选分支（A/B/C/D），分别导向浪漫、俏皮、策略推进、日常出人意料等不同风格
- 📊 **游戏状态追踪** — 好感度、心情、场景、时间段、剧情标记（flags）实时变化
- 🔧 **工具调用（可选）** — 开启后 AI 可以主动读写剧情设计文件（世界观、NPC 阵容、剧情大纲、进度追踪），实现有记忆的故事推进。可在行为设置中开关
- 📝 **多存档支持** — 创建多个对话存档，随时切换
- 💾 **JSON 备份** — 一键导出/导入完整游戏数据（不含 API Key）
- 🔐 **本地安全** — API Key 仅存储在系统级安全区域（Keychain/Keystore），绝不写入数据库或备份文件
- 🎨 **Material 3 设计** — 支持亮色/暗色主题自动切换
- 🌐 **多种 AI 后端** — 兼容任意 OpenAI 格式的 API（DeepSeek、OpenAI、Ollama 等均可）
...
3. 填入你的 **API Key**
4. 选择 **Model**（如 `deepseek-chat`）
5. 点击 **测试连接**

推荐使用 [DeepSeek](https://deepseek.com) 获得最佳中文 Galgame 体验。
...
<p align="center">
  <sub>本项目完全由 AI Agent 完成 · DeepSeek V4 Pro 85% · GPT 5.5 10% · Claude Opus 4.8 5%</sub>
</p>
