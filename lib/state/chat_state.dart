...
const String compressedContextMarker = '[上下文摘要]';

({String systemPrompt, List<Map<String, String>> history}) buildContextPayload({
  required String systemPrompt,
  required List<Message> messages,
  required AiSettings settings,
}) {
  final summaries = <String>[];
  final nonSystem = <Message>[];

  for (final message in messages) {
    if (message.role == 'system') {
      final content = message.content.trim();
      if (content.startsWith(compressedContextMarker)) {
  ...
  if (settings.truncateStrategy == 'truncate') {
    final maxMessages = settings.truncateLimit * 2;
  ...
  final effectiveSystemPrompt = summaries.isEmpty
      ? systemPrompt
      : '$systemPrompt\n\n## 已压缩的对话上下文\n\n${summaries.join('\n\n')}';
...
class ChatNotifier extends ChangeNotifier {
  ...
  /// Build a preview of the system prompt without user conversation text.
  Future<String> buildFullPrompt() async {
    ...
    final prompt = payload.systemPrompt;
    ...
    buf.writeln();
    buf.writeln('══════ 总计 ══════');
    buf.writeln('System Prompt: ${prompt.length} 字符');
    buf.writeln('预计 Tokens: ~${(prompt.length * 0.5).round()} (中英文混合估算)');
  ...
  Future<void> sendMessage(String text) async {
    ...
    try {
      ...
      final allHistory = [...contextPayload.history];
