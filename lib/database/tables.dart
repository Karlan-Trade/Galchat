...
class Conversations extends Table {
  ...
  IntColumn get characterId =>
      integer().named('character_id').references(Characters, #id)();
...
class Messages extends Table {
  ...
  IntColumn get conversationId =>
      integer().named('conversation_id').references(Conversations, #id)();
  TextColumn get role => text()(); // user | assistant | system | error
  TextColumn get speaker =>
      text().nullable()(); // character name for assistant messages
  TextColumn get content => text()();
  TextColumn get rawPayload =>
      text().named('raw_payload').nullable()(); // raw AI JSON
  TextColumn get reasoningContent =>
      text().named('reasoning_content').nullable()();
  DateTimeColumn get createdAt => dateTime().named('created_at')();
}
...
class Choices extends Table {
  ...
  IntColumn get messageId =>
      integer().named('message_id').references(Messages, #id)();
...
class GameStates extends Table {
  ...
  TextColumn get timeSlot =>
      text().named('time_slot').withDefault(const Constant('放学后'))();
  TextColumn get flagsJson =>
      text().named('flags_json').withDefault(const Constant('{}'))();
  DateTimeColumn get updatedAt => dateTime().named('updated_at')();
}
...
class AiSettings extends Table {
  ...
  IntColumn get maxTokens => integer()
      .named('max_tokens')
      .withDefault(const Constant(4096))(); // per-reply output cap
  IntColumn get contextWindow => integer()
      .named('context_window')
      .withDefault(const Constant(128000))(); // model's max context length
  TextColumn get truncateStrategy => text()
      .named('truncate_strategy')
      .withDefault(const Constant('compress'))(); // compress | truncate
  IntColumn get truncateLimit => integer()
      .named('truncate_limit')
      .withDefault(const Constant(20))(); // turns when truncate
  BoolColumn get markdownRender =>
      boolean().named('markdown_render').withDefault(
          const Constant(false))(); // enable markdown rendering in chat bubbles
  DateTimeColumn get updatedAt => dateTime().named('updated_at')();
}
